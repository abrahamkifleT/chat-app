    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ""}`}>
      <div className='pb-5'>
        ...
        <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
          ...
          <input onChange={(e) => setInput(e.target.value)} type="text" placeholder='Search' className='bg-transparent border-none outline-none text-white text-xs placeholder-[#c8c8c8] flex-1' />
        </div>
      </div>
      ...
      <div className='flex flex-col'>
        {
          filteredUser.map((user, index) => (
            <div onClick={() => { setSelectedUser(user); setUnseenMessages(prev => ({ ...prev, [user._id]: 0 })) }}
              ...
              <img src={user?.profilePic || assets.avatar_icon} alt="user" className='w-[35px] aspect-square rounded-full' />
