  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        <img src={selectedUser?.profilePic || assets.avatar_icon} alt="" onClick={() => setSelectedUser(false)} className='w-8 rounded-full' />
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {
          messages.map((message, index) => {
            ...
            return (
              <div key={index} className={`flex items-end gap-2 justify-end ${!isMyMessage && "flex-row-reverse"}`}>
                {
                  message.image ? (
                    <img src={message.image} alt="" className='max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-8' />
                  ) : (
                    <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-violet-500/30 text-white ${isMyMessage ? "rounded-br-none" : "rounded-bl-none"}`}>
                      {message.text}
                    </p>
                  )
                }
                ...
                <div className='text-center text-xs'>
                  <img src={isMyMessage ? authUser?.profilePic || assets.avatar_icon : selectedUser?.profilePic || assets.avatar_icon} alt="" className='w-7 rounded-full' />
